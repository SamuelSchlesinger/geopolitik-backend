...
data instance  Response LatestDraft = LatestDraftFound (Key Draft)
                                                       (Key User) Text UTCTime
                                    | LatestDraftNotFound
                                        deriving stock (Eq, Ord, Show, Read, Generic)
                                        deriving anyclass (FromJSON, ToJSON)

newtype DraftComments =
  DraftComments
...
data instance 
     Response DraftComments = DraftCommentsFound [Comment]
                                deriving stock (Eq, Ord, Show, Read, Generic)
                                deriving anyclass (FromJSON, ToJSON)
